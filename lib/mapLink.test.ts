 ...
 * кусками. Битое экранирование не повод не открыть карту — место в
 * ссылке уже разобрано, и оно важнее подсветки.
 */
describe('битая ссылка', () => {
  it('не роняет разбор, место открывается', () => {
    const v = parseMapHash('#12/52.5/69.25/%E0%A4%A');
    expect(v).toMatchObject({ zoom: 12, lat: 52.5, lon: 69.25 });
  });

  it('подсветку в таком случае оставляет как есть', () => {
    expect(parseMapHash('#12/52.5/69.25/%E0%A4%A')?.focus).toBe('%E0%A4%A');
  });

  it('целое экранирование по-прежнему разбирает', () => {
    expect(parseMapHash('#12/52.5/69.25/%D0%9C%D1%83%D1%84%D1%82%D0%B0')?.focus)
      .toBe('Муфта');
  });

  it('одинокий процент в конце не мешает', () => {
    expect(parseMapHash('#12/52.5/69.25/муфта%')).toMatchObject({ lat: 52.5 });
  });
});
