...
export interface WorkEntryBase {
  ...
  note?: string;
  /**
   * Кто закрыл день. Это инженер на объекте, а не отчётность: журнал
   * общий, но запись одна и у неё есть автор. Отчётность день не пишет —
   * она подтверждает исправления.
   */
  author?: string;
  /** Кто подал исправление, которое отчётность подтвердила. */
  editedBy?: string;
  /**
   * Заказчик с записью не согласен.
   *
   * Спорную строку нельзя ни удалять, ни молча править: она нужна
   * ровно в том виде, в каком её предъявляли. Но и в общем списке она
   * не должна теряться — поэтому помечается, а не прячется.
   */
  disputed?: boolean;
  /** В чём спор: «не приняли 320 м», «нет подписи технадзора». */
  disputeNote?: string;
  /**
   * Когда смену предъявили технадзору — ISO.
   *
   * Это не то же самое, что «записана»: между записью и предъявлением
   * проходят дни, а спрашивают потом именно про предъявление. Отдельной
   * тетради для этого вести не надо — отметка живёт в самой смене.
   */
  presentedAt?: string;
  /** Кому предъявили: фамилия технадзора. */
  presentedTo?: string;
  /**
   * Погода дня.
   *
   * В АСР есть такая графа, и её заполняют по памяти. Между тем дождь и
...
// ── Эксплуатация и аварии (Слой 3 в работе) ──────────────────────────────────

/**
 * Причина повреждения.
 *
 * Список взят из того, что рвёт кабель на самом деле. Он нужен не для
 * отчётности: если по одному месту трижды стоит «экскаватор», значит там
 * нет предупредительных знаков, а не «не везёт».
 */
export type IncidentCause =
  | 'экскаватор' | 'грызуны' | 'обрыв опоры' | 'кража'
  | 'износ' | 'ДТП' | 'пожар' | 'подтопление'
  | 'земляные работы' | 'вандализм' | 'неизвестно';

export const INCIDENT_CAUSES: IncidentCause[] = [
  'экскаватор', 'земляные работы', 'грызуны', 'обрыв опоры', 'кража',
  'вандализм', 'износ', 'ДТП', 'пожар', 'подтопление', 'неизвестно',
];

/**
 * Авария на сети.
 *
 * Запись о стройке перестаёт быть архивом именно здесь: у аварии есть
 * место, и по этому месту уже известно, на какой глубине там лежит труба,
 * каким способом её клали и кто. Через три года это единственный способ
 * понять, почему рвётся именно тут.
 */
export interface Incident {
  id: string;
  /** Когда заявили — ISO. */
  reportedAt: string;
  /** Когда устранили. Пусто — авария открыта. */
  fixedAt?: string;
  lat: number;
  lon: number;
  oblast?: string;
  ...
  uchastok?: string;
  kato?: string;
  /** Что повреждено: «ОК-24, 2 волокна», «муфта №3». */
  damage: string;
  /** Кто заявил — житель, аким, оператор. */
  reporter?: string;
  cause?: IncidentCause;
  /** Кто устранял. */
  crew?: string;
  contractor?: string;
  /** Часы на устранение. Если не проставлены, считаются по времени. */
  hours?: number;
 ...
 * Полевое фото.
 *
 * Снимок без координат и времени — это картинка, которую через месяц
 * никто не привяжет ни к участку, ни к дню. Поэтому у фото всегда есть
 * место и время, и всегда написано, откуда они взялись: из самого файла
 * (снято камерой там) или с телефона в момент, когда фото прикрепляли
 * (телефон мог быть уже в вагончике). Разница существенная, и прятать
 * её за общим словом «геотег» нельзя.
 *
 * Сам файл в журнале не лежит: он тяжёлый и хранится отдельно — локально
 * до обмена, в облаке после. Здесь только то, что нужно, чтобы его найти
 * и понять, что на нём.
 */
export type PhotoGeoSource = 'exif' | 'device' | 'manual' | 'none';

export interface FieldPhoto {
  id: string;
  /** К чему приложено. */
  kind: 'entry' | 'object' | 'deviation' | 'drill';
  /** id записи, объекта, отклонения или прокола. */
  refId: string;
  lat?: number;
  lon?: number;
  /** Откуда взялись координаты — это не одно и то же. */
  geoSource: PhotoGeoSource;
  /** Когда снято (EXIF) или когда приложено. */
  takenAt: string;
  /** Время из EXIF, если оно там было. */
  exifAt?: string;
  oblast?: string;
  rayon?: string;
  uchastok?: string;
  ...
  note?: string;
  author?: string;
  /** Ссылка в облаке — появляется после обмена. */
  url?: string;
  /** Путь в хранилище, чтобы можно было удалить. */
  storagePath?: string;
  /** Файл лежит локально и ещё не ушёл в облако. */
  pending?: boolean;
  /** Размер файла, байты — чтобы видеть, сколько ждёт отправки. */
  bytes?: number;
  createdAt: string;
  updatedAt: string;
  sync?: 'local' | 'synced';
...
export interface Crew {
  ...
  note?: string;
  /**
   * Выведена по журналу, а не заведена руками. Состава и техники у такой
   * колонны нет — в дневном отчёте их не пишут, — поэтому на карте она
   * пунктиром: это счёт бригад, а не карточка бригады.
   */
  derived?: boolean;
  updatedAt: string;
}
