...
// Склоняются оба слова: «узел связи», «узла связи», «узлом связи».
// Требовать небукву сразу за корнем «связ» значит не узнать самое
// обычное название узла и положить его на карту камерой.
const RE_OLT          = new RegExp(`${NONL}(?:olt|цод|амс|(?:узел|узл[а-яё]+)[\\s_-]*связ[а-яё]*|data[\\s-]?center)${NONR}|magistral`, 'i');
// Окончание прилагательного перечислять буквами нельзя: «транзитная»
// кончается на «я», которой в перечне не было, и стандартное название
// уходило в простые муфты. (В перечне заодно жила латинская «a» среди
// кириллицы — верный признак, что чинили следствие, а не причину.)
const RE_TB           = new RegExp(`мток|омсп|транзит[а-яё]*[\\s_-]+муфт|${NONL}(?:tb|sleeve)${NONR}`, 'i');
