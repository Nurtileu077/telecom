...
import { isGenericSheetName, sanitizeDescription } from '@/lib/labels';
...
function kmlPointTitle(p: KmlPoint): string {
  const name = sanitizeDescription(p.name);
  if (name && !isGenericSheetName(name)) return name;
  const desc = sanitizeDescription(p.desc);
  if (desc && !isGenericSheetName(desc)) return desc;
  return '';
}
  ...
  for (const [districtName, b] of byDistrict.entries()) {
    ...
    const tbs: TransitBox[] = tbInput.map((t, i) => {
      ...
      return {
        ...
        inCable: 'ОК-48',
        outCable: 'ОК-4',
        muftaType: (title.match(/мток[-\s]*(\d+)/i)?.[0] ?? 'МТОК-96А') as TransitBox['muftaType'],
      };
    });
    stats.tb += tbs.length;
    ...
    const subs: Subscriber[] = b.subs.map((s, i) => {
      ...
      return {
        id: `sub-${slug}-${i + 1}`,
        lat: s.lat, lon: s.lon,
        desc: title || sanitizeDescription(s.desc) || `Абонент ${i + 1}`,
        district: districtName,
        orkId: nearest?.id,
    ...
    // Боксы ОНТ — рядом с абонентом (или новая точка, если абонента нет).
    for (const box of b.boxes) {
      const title = kmlPointTitle(box) || 'Бокс (ОНТ)';
      let bestSub: Subscriber | null = null;
      let bestD = Infinity;
      for (const s of subs) {
        const d = haversineM(box.lat, box.lon, s.lat, s.lon);
        if (d < bestD) { bestD = d; bestSub = s; }
      }
      if (bestSub && bestD <= 25) {
        if (!bestSub.desc.includes(title)) {
          bestSub.desc = `${title} · ${bestSub.desc}`;
        }
      } else {
        subs.push({
          id: `sub-${slug}-box-${subs.length + 1}`,
          lat: box.lat,
          lon: box.lon,
          desc: title,
          district: districtName,
    ...
    for (const line of b.lines) {
      ...
      const lengthM = resolveCableLengthM(line.coords, {
        name: line.name,
        extData: line.extData,
      });
      outCables.push({
