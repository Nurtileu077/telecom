import type { Cable, District, OLT, ORK, Subscriber, TransitBox } from '@/types/network';
...
export type InteriorKind = 'olt' | 'tb' | 'ork';
...
export type PeerKind = 'olt' | 'tb' | 'ork' | 'sub' | 'unknown';
...
export type EntityInteriorData = OltInteriorData | TbInteriorData | OrkInteriorData;

function peerKindForId(districts: District[], id: string): PeerKind {
...
export function cableLinksForEntity(
  ...
  return cablesForEntity(cables, entityId).map((c) => {
    ...
    const pk = peerKindForId(districts, peerId);
    return {
      cable: c,
...
export function resolveInterior(
  ...
  if (view.kind === 'olt') {
    ...
    const links = cableLinksForEntity(olt.id, cables, districts);
  ...
  if (view.kind === 'tb') {
    ...
    if (!tb) return null;
    const links = cableLinksForEntity(tb.id, cables, districts);
  ...
  if (!ork) return null;
  const links = cableLinksForEntity(ork.id, cables, districts);
