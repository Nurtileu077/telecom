...
import {
  ...
  isTrunkishCable,
} from './CablePhysicalSegments';

export { consolidateTrunksForMaterialsAccounting } from './CablePhysicalSegments';
...
export function calculateMaterials(
  ...
  const nonOk4Before = cables.filter(isTrunkishCable).length;
  const nonOk4After = cablesForSum.filter(isTrunkishCable).length;
