...
import { AlertTriangle, MapPin, Trash2, Check } from 'lucide-react';
...
export default function ChecksView({ journal, onShow, onDeleteRoute }: Props) {
  const pairs = useMemo(
  ...
  if (journal.planRoutes.length === 0) {
    return (
      <div className="p-6 text-center text-[13px] text-[var(--text-muted)]">
        Трасс пока нет. Загрузите KML — и проверки заработают.
      </div>
    );
  }
